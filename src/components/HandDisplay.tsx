...
const TileComponent: React.FC<{
  ...
  const baseClasses = 'inline-flex items-center justify-center min-w-[2.5rem] h-14 px-2 border-2 rounded text-lg font-bold transition-all';
 ...
 * 要件 2.1, 2.5 を満たす
 */
export const HandDisplay: React.FC<HandDisplayProps> = ({ hand, winningTile }) => {
  const actualWinningTile = winningTile || hand.winningTile;

  return (
    <div className="w-full max-w-4xl mx-auto p-4 bg-gradient-to-b from-green-50 to-green-100 rounded-lg shadow-md">
      {/* 手牌情報 */}
      ...
      <div className="space-y-4">
        {/* 鳴き */}
        {hand.melds.length > 0 && (
          <div className="flex flex-wrap gap-3 pb-3 border-b border-green-300">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {hand.closedTiles.map((tile, index) => (
            <TileComponent key={index} tile={tile} />
          ))}
        </div>

        {/* 和了牌 */}
        <div className="flex flex-wrap gap-2 items-center pt-2 border-t border-green-300">
